...
def create_booking(
    ...
    # Live (confirmed) bookings still hold their spot; a cancelled one frees up
    # capacity. The horse row lock taken above makes this sum-then-insert
...
def update_booking(
    ...
    # Cancellation is the only update left: bookings auto-confirm at creation
    # (routers/bookings.py's create_booking), so there's no pending state left
    # for a farm owner to confirm/decline into.
